...
def main() -> int:
    ...
    for facts, now, should, why in CASES:
        ...
        if not ok:
            bad += 1
        print(f"{'ok  ' if ok else 'FAIL'}  spoke={got!s:<5} want={should!s:<5}  {why}")

    print()
    if bad:
        print(f"{bad} of {len(CASES)} failed")
    else:
        print(f"all {len(CASES)} passed — the Saturday remark cannot happen again")
