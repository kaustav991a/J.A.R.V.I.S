...
# `lat`/`lon` are NOT optional decoration. `_clean_commute` reads them with
# `float(r.get("lat"))`, so a row without them raises TypeError and is dropped —
# and this fixture was written without them, so `_clean_commute(GOOD)` returned
# `departures: []` before a single deploy happened. Three tests asserted a
# schedule survived and were really asserting that an empty list was truthy; six
# more passed over a list that was always empty. None of it had ever been run by
# an interpreter holding the real function.
GOOD = {"tz": "Asia/Kolkata", "days": WEEK,
        "departures": [{"label": "Office", "hour": 19, "minute": 0,
                        "place_id": "office", "lat": 22.7900, "lon": 88.3700}]}
...
def test_a_departure_with_no_place_is_dropped_on_restore():
    async def go():
        _DB["commute"] = json.dumps({
            ...
            "departures": [{"label": "Office", "hour": 19, "minute": 0,
                            "place_id": "office", "lat": 22.79, "lon": 88.37},
                           # no place_id: this is the row that must be dropped,
                           # and it carries coordinates so that the DROP is
                           # attributable to the missing place and nothing else
