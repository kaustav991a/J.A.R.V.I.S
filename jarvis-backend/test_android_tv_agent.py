...
@pytest.fixture
def agent_connected() -> AndroidTVAgent:
    tv = AndroidTVAgent(ip_port="192.168.0.10:5555")
    tv._connected = True
    return tv


def test_tv_volume_parses_pipe_steps(agent_connected: AndroidTVAgent) -> None:
...
def test_tv_volume_action_engine_pipe_contract(agent_connected: AndroidTVAgent) -> None:
...
def test_tv_launch_app_youtube_package(agent_connected: AndroidTVAgent) -> None:
...
def test_connect_falls_back_to_env_ip_when_discovery_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("JARVIS_TV_IP", "192.168.99.1:40123")
...
def test_connect_prefers_mdns_over_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("JARVIS_TV_IP", "10.10.10.10:5555")
...
def test_discover_prefers_jarvis_tv_name_match(monkeypatch: pytest.MonkeyPatch) -> None:
    """Exercise zeroconf callback logic with patched browser + sleep."""
    monkeypatch.setenv("JARVIS_TV_NAME", "2KTV-3MH")
    ...
    def browser_side_effect(zc, stype, handlers=None):
        if handlers:
            handlers[0](zc, stype, "generic._adb-tls-connect._tcp.local.", ServiceStateChange.Added)
            handlers[0](zc, stype, "2KTV-3MH._adb-tls-connect._tcp.local.", ServiceStateChange.Added)
        return MagicMock()
