...
|---|---|---|
| Automatic suite | **101 harnesses, 3423 checks, 0 failed** | `jarvis-backend\venv\Scripts\python.exe run_harnesses.py` — the system python fakes failures |
...
| 0.2 | Fix `GEMINI_API_KEY` — and understand the quota | ✅ **done 2026-08-22 (evening).** **The earlier entry here was STALE and is corrected:** it said the primary key was `400 API key not valid` and that five keys were configured. Re-measured after he updated `.env` — **4 keys configured, and all 4 credentials are ACCEPTED.** Nothing is invalid, so there is nothing to fix. All 4 were `429 RESOURCE_EXHAUSTED` for generation at the time of measuring, and **4 of those calls were mine** while establishing this — the quota is a shared bucket per Google project, so more keys in the same project still multiply nothing. The machine half is now closed too: boot **names a rejected credential per key** and separates `invalid` from `exhausted`, because those need opposite responses and the log used to render both as "catalogue unreachable" — the tier 0.4 mistake in a second place | **SEALED** — harness `test_boot_preflight.py` **84 checks** (was 47) · negative-tested live with a bogus key (`CREDENTIAL REJECTED`), with a bogus key FIRST and a real one behind it (`#1 of 2 rejected · 1 of 2 still accepted` — the case that used to hide four good keys), and with no key at all · live boot prints `all 11 configured model id(s) exist` + `all 4 Gemini key(s) accepted` · one bug caught in my own check before it became a finding: routing per-key probes through `_default_fetch` doubled the `?key=` and returned 400 for every key, which reads exactly like four dead keys |
...
| **0.2** the Gemini keys | ✅ | harness 84 checks · all three branches negative-tested live (bogus key, bogus-key-first-real-second, no key) · the stale claim in this tracker was **corrected by re-measuring**: 4 keys, all valid, all exhausted, one shared bucket |
